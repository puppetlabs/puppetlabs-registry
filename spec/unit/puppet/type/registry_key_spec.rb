  ...
  describe "#eval_generate" do
    ...
    context "purging" do
      ...
      before :each do
        ...
        catalog.add_resource(Puppet::Type.type(:registry_value).new(:path => "#{key[:path]}\\", :value_name => "val1", :catalog => catalog))
        catalog.add_resource(Puppet::Type.type(:registry_value).new(:path => "#{key[:path]}\\", :value_name => "val2", :catalog => catalog))
      end
